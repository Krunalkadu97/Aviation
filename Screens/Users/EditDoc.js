...
// create a component
const EditUDoc = ({ navigation,route }) => {
    ...
    useEffect(() => {
        let {  dosc,ro } = route.params;

       
       setSelectedDoc(dosc.USD_NAME);
       setDno(dosc.USD_DOCUMENT_NO);
        setIdat(new Date(dosc.USD_ISSUE_DATE));
        setEdat(new Date(dosc.USD_EXPIRY_DATE));
        setImage(dosc.USD_FILE);
        setRole(ro);
