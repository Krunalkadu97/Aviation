...
// create a component
const EditAddUs = ({navigation,route}) => {
    ...
    useEffect(()=>{
        let {item,role} = route.params;

        setType(item.USA_ADDRESS_TYPE);
        setAddress(item.USA_ADDRESS);
        setCity(item.USA_CITY);
        setStates(item.USA_STATE);
        setCountry(item.USA_COUNTRY);
        setPCode(item.USA_ZIPCODE);
        setUser(role.US_RO_ID);
    },[]);
    return (
